...
<x-layouts::shop :title="__('Eventos')">
    ...
    <section class="bg-[#1e2e74]">
        <div class="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 py-10 lg:flex-row lg:items-center lg:justify-between lg:px-8">
            <div class="space-y-1">
                <p class="text-xs font-semibold uppercase tracking-[0.25em] text-[#f5a520]">{{ __('Agenda Mikrokosmos') }}</p>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <form method="GET" action="{{ route('shop.events.index') }}" class="flex items-center gap-2">
                    <input
                        ...
                        class="rounded-full border border-white/20 bg-white/10 px-3 py-2 text-sm text-white focus:border-[#f5a520] focus:outline-none"
    ...
    <section class="bg-[#f8f7ff]">
        <div class="mx-auto w-full max-w-6xl px-4 py-10 lg:px-8">
            ...
            @if (($errors->eventRegistration ?? null) && $errors->eventRegistration->any())
                <div class="mb-6 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-800">
            ...
            <div class="mb-2 hidden grid-cols-7 text-center text-xs font-semibold uppercase tracking-[0.2em] text-[#8b8fcc] md:grid">
            ...
            <div class="mb-4 space-y-3 md:hidden">
                <div class="flex items-center justify-between">
                    <a
                        ...
                        class="rounded-full border border-[#dddeff] px-3 py-1.5 text-sm font-semibold text-[#6b70c4] transition hover:border-[#8b8fcc] hover:bg-white"
                        wire:navigate
                    >&larr; {{ __('Semana') }}</a>
                ...
                <div class="space-y-2">
                    @foreach ($mobileWeek as $date)
                        ...
                        <a
                            ...
                            class="block rounded-xl border p-3 text-sm transition hover:-translate-y-0.5 {{ $isSelected ? 'border-[#6b70c4] bg-[#eeeeff]' : ($isToday ? 'border-[#f5a520] bg-[#fff8eb]' : 'border-[#dddeff] bg-white hover:border-[#8b8fcc]') }}"
                            wire:navigate
                        >
                            <div class="flex items-center gap-2">
                                <span class="text-xs font-semibold {{ $isSelected ? 'text-[#6b70c4]' : ($isToday ? 'text-[#c47a00]' : 'text-[#8b8fcc]') }}">
                                    {{ $date->translatedFormat('D') }}
                                </span>
                                <span class="text-sm font-bold {{ $isSelected ? 'text-[#4a4fa8]' : 'text-[#1e2e74]' }}">
                            ...
                            @if ($dayEvents->isNotEmpty())
                                <div class="mt-2 flex flex-wrap gap-1">
                                    @foreach ($dayEvents->take(2) as $event)
                                        <span class="rounded-md bg-[#6b70c4] px-2 py-0.5 text-[10px] font-semibold text-white">
                                            {{ $event->name }}
                                        </span>
                                    @endforeach
                                    @if ($dayEvents->count() > 2)
                                        <span class="text-[10px] font-semibold text-[#8b8fcc]">+{{ $dayEvents->count() - 2 }}</span>
                                    @endif
                                </div>
                            @endif
                        </a>
                ...
                @foreach ($calendarWeeks as $week)
                    <div class="grid grid-cols-7 gap-1">
                        @foreach ($week as $date)
                            ...
                            <a
                                href="{{ route('shop.events.index', ['month' => $currentMonth->format('Y-m'), 'day' => $date->toDateString()]) }}"
                                class="min-h-[110px] rounded-xl border p-2.5 text-sm transition hover:-translate-y-0.5 {{ $isSelected ? 'border-[#6b70c4] bg-[#eeeeff]' : ($isToday ? 'border-[#f5a520] bg-[#fff8eb]' : 'border-[#dddeff] bg-white hover:border-[#8b8fcc]') }} {{ !$isCurrentMonth ? 'opacity-40' : '' }}"
                                wire:navigate
                            >
                                <div class="flex items-center justify-between">
                                    <span class="text-sm font-bold {{ $isSelected ? 'text-[#6b70c4]' : ($isToday ? 'text-[#c47a00]' : 'text-[#1e2e74]') }}">
                                        {{ $date->translatedFormat('j') }}
                                    </span>
                                    @if ($isToday)
                                ...
                                <div class="mt-2 space-y-1">
                                    @foreach ($dayEvents->take(3) as $event)
                                        <div class="truncate rounded-md bg-[#6b70c4] px-1.5 py-0.5 text-[10px] font-semibold text-white">
                                            {{ $event->start_at->format('H:i') }} {{ $event->name }}
                                        </div>
                                    @endforeach
                                    @if ($dayEvents->count() > 3)
                                        <div class="text-[10px] font-semibold text-[#8b8fcc]">+{{ $dayEvents->count() - 3 }}</div>
                                    @endif
                                </div>
                            </a>
                        @endforeach
            ...
            <div id="day-events-modal" class="fixed inset-0 z-40 {{ $selectedDate ? '' : 'hidden' }}">
                <button
                    type="button"
                    class="absolute inset-0 h-full w-full bg-[#1e2e74]/50"
                ...
                <div class="relative mx-auto mt-12 flex w-full max-w-2xl rounded-2xl border border-[#dddeff] bg-white px-5 py-6 shadow-2xl">
                    <button
                        type="button"
                        class="absolute right-4 top-4 flex size-7 items-center justify-center rounded-full bg-[#f0eeff] text-[#6b70c4] transition hover:bg-[#dddeff]"
                        onclick="window.location.href='{{ route('shop.events.index', ['month' => $currentMonth->format('Y-m')]) }}'"
                        aria-label="{{ __('Cerrar') }}"
                    >✕</button>
                    ...
                    <div class="max-h-[85vh] w-full overflow-y-auto pr-1">
                        @if ($selectedDate)
                            <div class="mb-6">
                                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-[#8b8fcc]">{{ __('Eventos') }}</p>
                            ...
                            <div class="space-y-4">
                                    ...
                                    <article class="rounded-xl border border-[#dddeff] bg-[#f8f7ff] p-4">
                                        <div class="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
                                            <div>
                                                ...
                                                <p class="text-sm text-[#8b8fcc]">{{ $event->eventType?->name ?? __('Actividad de comunidad') }}</p>
                                            </div>
                                            <div class="shrink-0 text-sm text-[#4a4fa8]/70">
                                        ...
                                        @endphp
                                        <div class="mt-2 flex flex-wrap items-center gap-2 text-xs font-semibold text-[#8b8fcc]">
                                        ...
                                        @if ($event->description)
                                            <p class="mt-2 text-sm text-[#4a4fa8]/70">{{ \Illuminate\Support\Str::limit($event->description, 200) }}</p>
                                        @endif
                                            ...
                                            @auth('shop')
                                                ...
                                                @else
                                                    <form method="POST" action="{{ route('shop.events.register', $event) }}">
                                                        @csrf
                                                        <button type="submit" class="rounded-full bg-[#f5a520] px-5 py-2 text-sm font-semibold text-[#1e2e74] transition hover:bg-[#ffd978]">
                                                            {{ __('Registrarme') }}
                                                        </button>
                                                    </form>
                                                @endif
                                ...
                                @empty
                                    <div class="rounded-xl border border-[#dddeff] bg-white p-5">
                                        ...
                                        <p class="mt-1 text-sm text-[#8b8fcc]">{{ __('¿Quieres proponer algo? Completa el formulario y lo revisaremos.') }}</p>

                                        <form method="POST" action="{{ route('shop.events.suggest') }}" class="mt-5 space-y-3">
                                            @csrf
                                            <input type="hidden" name="date" value="{{ $selectedDate->toDateString() }}">
                                            <div>
                                                <label for="proposal-name" class="mb-1 block text-xs font-semibold text-[#6b70c4]">{{ __('Tu nombre') }}</label>
                                                <input id="proposal-name" name="name" type="text" required value="{{ old('name') }}"
                                                    class="w-full rounded-lg border border-[#dddeff] px-3 py-2 text-sm text-[#1e2e74] focus:border-[#6b70c4] focus:outline-none">
                                            </div>
                                            <div>
                                                <label for="proposal-email" class="mb-1 block text-xs font-semibold text-[#6b70c4]">{{ __('Correo') }}</label>
                                                <input id="proposal-email" name="email" type="email" required value="{{ old('email') }}"
                                                    class="w-full rounded-lg border border-[#dddeff] px-3 py-2 text-sm text-[#1e2e74] focus:border-[#6b70c4] focus:outline-none">
                                            </div>
                                            <div>
                                                <label for="proposal-title" class="mb-1 block text-xs font-semibold text-[#6b70c4]">{{ __('Título del evento') }}</label>
                                                <input id="proposal-title" name="title" type="text" required value="{{ old('title') }}"
                                                    class="w-full rounded-lg border border-[#dddeff] px-3 py-2 text-sm text-[#1e2e74] focus:border-[#6b70c4] focus:outline-none">
                                            </div>
                                            <div>
                                                <label for="proposal-description" class="mb-1 block text-xs font-semibold text-[#6b70c4]">{{ __('Detalles') }}</label>
                                                <textarea id="proposal-description" name="description" rows="3" required
                                                    class="w-full rounded-lg border border-[#dddeff] px-3 py-2 text-sm text-[#1e2e74] focus:border-[#6b70c4] focus:outline-none">{{ old('description') }}</textarea>
                                            </div>
                                            <button type="submit" class="rounded-full bg-[#f5a520] px-5 py-2 text-sm font-semibold text-[#1e2e74] transition hover:bg-[#ffd978]">
                                                {{ __('Enviar propuesta') }}
                                            </button>
                        ...
                        @else
                            <p class="text-sm text-[#8b8fcc]">{{ __('Selecciona un día para ver los eventos.') }}</p>
                        @endif
                    </div>
                </div>
            </div>
