...
<x-layouts::shop :title="__('Eventos')">
    ...
    <section class="bg-[#f8f7ff]">
        <div class="mx-auto w-full max-w-6xl px-4 py-10 lg:px-8">
            ...
            <div class="mb-2 hidden grid-cols-7 text-center text-xs font-semibold uppercase tracking-[0.2em] text-[#b184db] md:grid">
                ...
                <div class="space-y-2">
                    @foreach ($mobileWeek as $date)
                        ...
                        <a href="{{ route('shop.events.index', ['month' => $currentMonth->format('Y-m'), 'day' => $date->toDateString()]) }}"
                            class="block rounded-xl border p-3 text-sm transition hover:-translate-y-0.5 {{ $isSelected ? 'border-[#b184db] bg-[#eeeeff]' : ($isToday ? 'border-[#f5a520] bg-[#fff8eb]' : 'border-[#dddeff] bg-white hover:border-[#b184db]') }}"
                            wire:navigate>
                            <div class="flex items-center gap-2">
                                <span class="text-xs font-semibold {{ $isSelected ? 'text-[#b184db]' : ($isToday ? 'text-[#c47a00]' : 'text-[#4a4fa8]') }}">{{ $date->translatedFormat('D') }}</span>
                                <span class="text-sm font-bold text-[#8a2ab6]">{{ $date->translatedFormat('j') }}</span>
                                @if ($isToday)
                                    <span class="ml-auto rounded-full bg-[#f5a520] px-2 py-0.5 text-[10px] font-bold text-[#8a2ab6]">{{ __('Hoy') }}</span>
                                @endif
                            </div>
                            @if ($dayEvents->isNotEmpty())
                                <div class="mt-2 flex flex-wrap gap-1">
                                    @foreach ($dayEvents->take(2) as $event)
                                        <span class="rounded-md bg-[#b184db] px-2 py-0.5 text-[10px] font-semibold text-white">{{ $event->name }}</span>
                                    @endforeach
                ...
                @foreach ($calendarWeeks as $week)
                    <div class="grid grid-cols-7 gap-1">
                        @foreach ($week as $date)
                            ...
                            @endphp
                            <a href="{{ route('shop.events.index', ['month' => $currentMonth->format('Y-m'), 'day' => $date->toDateString()]) }}"
                                class="min-h-[110px] rounded-xl border p-2.5 transition hover:-translate-y-0.5 {{ $isSelected ? 'border-[#b184db] bg-[#eeeeff]' : ($isToday ? 'border-[#f5a520] bg-[#fff8eb]' : 'border-[#dddeff] bg-white hover:border-[#b184db]') }} {{ !$isCurrentMonth ? 'opacity-40' : '' }}"
                                wire:navigate>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm font-bold {{ $isSelected ? 'text-[#b184db]' : ($isToday ? 'text-[#c47a00]' : 'text-[#8a2ab6]') }}">{{ $date->translatedFormat('j') }}</span>
                                    @if ($isToday)
                                        <span class="rounded-full bg-[#f5a520] px-1.5 py-0.5 text-[9px] font-bold text-[#8a2ab6]">{{ __('Hoy') }}</span>
                                    @endif
                                </div>
                                <div class="mt-2 space-y-1">
                                    @foreach ($dayEvents->take(3) as $event)
                                        <div class="truncate rounded-md bg-[#b184db] px-1.5 py-0.5 text-[10px] font-semibold text-white">{{ $event->start_at->format('H:i') }} {{ $event->name }}</div>
                                    @endforeach
            ...
            <div id="day-events-modal" class="fixed inset-0 z-40 {{ $selectedDate ? '' : 'hidden' }}">
                <button type="button" class="absolute inset-0 h-full w-full bg-[#8a2ab6]/50" onclick="window.location.href='{{ route('shop.events.index', ['month' => $currentMonth->format('Y-m')]) }}'" aria-label="{{ __('Cerrar') }}"></button>
                ...
                <div class="relative mx-auto mt-12 flex w-full max-w-2xl rounded-2xl border border-[#dddeff] bg-white px-5 py-6 shadow-2xl">
                    ...
                    <div class="max-h-[85vh] w-full overflow-y-auto pr-1">
                        @if ($selectedDate)
                            <div class="mb-6">
                                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-[#b184db]">{{ __('Eventos') }}</p>
                                <h2 class="text-2xl font-bold text-[#8a2ab6]">{{ $selectedDate->translatedFormat('l j \\d\\e F') }}</h2>
                            </div>
                            ...
                            <div class="space-y-4">
                                @forelse ($selectedDayEvents as $event)
                                    <article class="rounded-xl border border-[#dddeff] bg-[#f8f7ff] p-4">
                                        <div class="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
                                            <div>
                                                <p class="text-lg font-bold text-[#8a2ab6]">{{ $event->name }}</p>
                                            ...
                                            @auth('shop')
                                                ...
                                                @else
                                                    <form method="POST" action="{{ route('shop.events.register', $event) }}">
                                                        @csrf
                                                        <button type="submit" class="rounded-full bg-[#f5a520] px-5 py-2 text-sm font-semibold text-[#8a2ab6] transition hover:bg-[#ffd978]">{{ __('Registrarme') }}</button>
                                                    </form>
                                                @endif
                                            @else
                                                <form method="POST" action="{{ route('shop.events.register', $event) }}">
                                ...
                                @empty
                                    <div class="rounded-xl border border-[#dddeff] bg-white p-5">
                                        <h3 class="font-bold text-[#8a2ab6]">{{ __('Sin eventos este día') }}</h3>
                                        ...
                                        <form method="POST" action="{{ route('shop.events.suggest') }}" class="mt-5 space-y-3">
                                            ...
                                            @foreach ([['proposal-name','name',__('Tu nombre'),'text'], ['proposal-email','email',__('Correo'),'email'], ['proposal-title','title',__('Título del evento'),'text']] as [$id,$name,$label,$type])
                                                <div>
                                                    ...
                                                    <input id="{{ $id }}" name="{{ $name }}" type="{{ $type }}" required value="{{ old($name) }}" class="w-full rounded-lg border border-[#dddeff] px-3 py-2 text-sm text-[#8a2ab6] focus:border-[#b184db] focus:outline-none">
                                                </div>
                                            @endforeach
                                            <div>
                                                <label for="proposal-description" class="mb-1 block text-xs font-semibold text-[#4a4fa8]">{{ __('Detalles') }}</label>
                                                <textarea id="proposal-description" name="description" rows="3" required class="w-full rounded-lg border border-[#dddeff] px-3 py-2 text-sm text-[#8a2ab6] focus:border-[#b184db] focus:outline-none">{{ old('description') }}</textarea>
                                            </div>
                                            <button type="submit" class="rounded-full bg-[#f5a520] px-5 py-2 text-sm font-semibold text-[#8a2ab6] transition hover:bg-[#ffd978]">{{ __('Enviar propuesta') }}</button>
                                        </form>
                                    </div>
                                @endforelse
                            </div>
