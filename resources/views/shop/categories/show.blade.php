<x-layouts::shop :title="$category->name">
    ...
    <section class="bg-[#1e2e74]">
        <div class="mx-auto w-full max-w-6xl px-4 py-14 lg:px-8">
            ...
            <p class="text-xs font-semibold uppercase tracking-[0.25em] text-[#f5a520]">{{ __('Colección') }}</p>
            ...
            @else
                <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                    @foreach ($products as $product)
                        <a href="{{ route('shop.products.show', $product) }}" class="group block rounded-2xl border border-[#dddeff] bg-white shadow-sm transition hover:-translate-y-1 hover:border-[#8b8fcc] hover:shadow-md" wire:navigate>
                            ...
                            <div class="p-5">
                                ...
                                @if ($product->description)
                                    <p class="mt-1 text-sm text-[#4a4fa8]/60">{{ \Illuminate\Support\Str::limit($product->description, 100) }}</p>
                                @endif
