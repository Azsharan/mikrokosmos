            ...
            @else
                <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                    @foreach ($products as $product)
                        <a href="{{ route('shop.products.show', $product) }}" class="group block rounded-2xl border border-[#dddeff] bg-white shadow-sm transition hover:-translate-y-1 hover:border-[#b184db] hover:shadow-md" wire:navigate>
                            ...
                            @endif
                            <div class="p-5">
                                <h3 class="font-bold text-[#8a2ab6] transition group-hover:text-[#b184db]">{{ $product->name }}</h3>
                                @if ($product->description)
                                    <p class="mt-1 text-sm text-[#4a4fa8]">{{ \Illuminate\Support\Str::limit($product->description, 100) }}</p>
                                @endif
                                <div class="mt-4 flex items-center justify-between">
                                    <span class="text-xl font-bold text-[#8a2ab6]">{{ number_format((float) $product->price, 2) }} €</span>
                                    <span class="text-sm font-semibold text-[#b184db] transition group-hover:translate-x-1">&rarr;</span>
                                </div>
                            </div>
                        </a>
                    @endforeach
